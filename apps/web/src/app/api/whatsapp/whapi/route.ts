...
import { extractAndSaveDraft, isJobMessage, sendWhapiText } from '@ddots/api/lib/import';
...
export async function POST(req: Request) {
  ...
  const rl = await rateLimit('import:whapi', 100, 3600);
  ...
  for (const m of messages) {
    ...
    if (text.length < 15 || !isJobMessage(text)) continue;
    try {
