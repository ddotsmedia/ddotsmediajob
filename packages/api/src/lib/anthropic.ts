...
export const WALKIN_EXTRACT_TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    type: 'object',
    properties: {
      title: { type: 'string', description: 'Job title/role. If multiple positions are listed, join the titles with " / " (e.g. "Sr. Estimator – Joinery / Sr. Estimator – Fit-Out"). Empty if unknown.' },
      company: { type: 'string', description: 'Hiring company, empty if unknown' },
      location: { type: 'string', description: 'Area / emirate of the role' },
      walkin_date: { type: 'string', description: 'First walk-in date as YYYY-MM-DD. Parse "25-06-2026", "25/06/2026", "June 25, 2026" -> YYYY-MM-DD. Empty if unknown.' },
      ...
      venue: { type: 'string', description: 'Venue / address. Extract ANY location mention — city, area, or building fragment. Never empty if any location text exists.' },
      contact_phone: { type: 'string', description: 'Any +971 or 05X phone/WhatsApp number found anywhere in the text. Empty if none.' },
      required_docs: { type: 'string', description: 'Documents to bring, from the Requirements/bring section — CV, passport copy, photo, certificates, etc. Empty if none explicitly stated as documents to bring.' },
