# DdotsMediaJobs — Codex Master Build File

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EXECUTION RULES — FOLLOW EXACTLY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
MODEL: Codex-haiku-4-5 for ALL tasks. No Sonnet. No Opus. Zero exceptions.
  If Haiku fails, retry with simpler approach. Never escalate.
OUTPUT: "done: [task name]" per completed task only.
  "blocked: [task name] - [reason]" then immediately next task.
  No recaps. No summaries. No explanations. No preamble. Just do it.
FLOW: Complete everything automatically. No stopping. No user input between tasks.
  Never stop for a blocked task — skip and continue.
CONTEXT: No re-reading files already read this session.
  No outputting file contents. No quoting code back. Keep context small.
EDITS: Batch all related file edits into single tool calls always.
BUILD: npm run build after every phase. Fix all errors before next phase.
  git add -A && git commit -m "feat: [phase]" after each phase.
  git push origin main every 5 commits.

START SEQUENCE (run automatically, no stopping):
  1. Read AGENTS.md
  2. Read TASKS.md
  3. Find every [ ] task in TASKS.md
  4. Build every unchecked task in order using Haiku only
  5. Mark completed [x] in TASKS.md
  6. npm run build after every phase
  7. Commit after each phase
  8. Continue until every task is [x]
  9. Then build additional features from AGENTS.md in order
  10. No stopping between any steps
...
Codex API: Codex-haiku-4-5 (simple) Codex-sonnet-4-6 (reasoning)
...
14. Validate all Codex API responses with Zod before DB write
...
job-alerts-worker.ts
  ...
  Action: semantic match against active alerts (Codex Haiku)
...
31. jobVoiceover → browser SpeechSynthesis API (no Codex needed)
...
- Multi-language CV parser:
    On CV upload → Codex Haiku extracts structured data
